...
def get_character_set_choice():
    ...
    while True:
        ...
        choices = input("Enter the numbers of your choices (e.g., 1234 for all): ")
        
        if not choices:
            print("Error: You must enter at least one choice. Please try again.")
            continue
        
        if not all(char in '1234' for char in choices):
            print("Error: Invalid input. Please use only numbers 1-4.")
            continue
